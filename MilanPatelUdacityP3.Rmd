...
aggdata <- aggregate (surveydata, by=list (surveydata$dist), FUN=mean,
      na.rm=TRUE) 
...
ggplot () + geom_map (data=districtmap, aes (x=long, y=lat,map_id=id), 
   ...
   geom_map (data = aggdata, aes (map_id = dist, fill = q4_7), 
   map = districtmap)+expand_limits (x = districtmap$long, y = districtmap$lat)
...
surveydata$ISIC4_desc <- isicdata$Description[match (surveydata$q1_2, 
  isicdata$Code)]
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=50}
...
ggplot (aes (x=ISIC_sectiondesc,y=q4_7),
  ...
  scale_x_discrete (labels = function (x) str_wrap (x, width = 15)) + ggtitle ("Boxplot -- Revenue by ISIC Section")
...
```{r echo=FALSE, options= TRUE, message=FALSE, warning=FALSE}
options(scipen=999)

surveydata_wisic<-subset(surveydata,!(is.na(ISIC_sectiondesc))|!(is.null(q4_7)|!(is.na(q4_7)|!(is.null(ISIC_sectiondesc)))))

#surveydata_wisic<-subset(surveydata_wisic,!(is.null(surveydata_wisic$q4_7)))
#surveydata_wisic<-subset(surveydata_wisic,!(is.na(surveydata_wisic$q4_7)))

#surveydata_wisic<-subset (surveydata_wisic,subset= ISIC_section%in% c ('D','G','H','O','I','K','N'))

#remove ghost categorizations
surveydata$ISIC_sectiondesc<-factor(surveydata$ISIC_sectiondesc)



#surveydata_wisic<-table(droplevels(surveydata_wisic)$ISIC_sectiondesc)


by (surveydata$q4_7,surveydata$ISIC_sectiondesc, summary)

###write.csv2(surveydata_wisic,"surveydata_wisic.csv")
#by (surveydata_wisic$q4_7,surveydata_wisic$ISIC_sectiondesc, summary)
...
ggplot (subset (surveydata, surveydata$ISIC_section=='H')) + 
  ...
  binwidth=500000)+scale_x_continuous (limits=c (0,15000000) )+
...
##plot histogram of annual revenue--retail repair

ggplot (subset (surveydata, surveydata$ISIC_section=='G')) + 
  geom_histogram (aes (q4_7), 
  binwidth=500000)+scale_x_continuous (limits=c (0,15000000) )+
  labs (x="Takas", title="Annual Revenue 2009- Retail & Repair")  
...
##plot histogram of annual revenue--manufact.

ggplot (subset (surveydata, surveydata$ISIC_section=='D')) + 
  geom_histogram (aes (q4_7), 
  binwidth=500000)+scale_x_continuous (limits=c (0,15000000) )+
  labs (x="Takas", title="Annual Revenue 2009- Manufacturing")  
...
##plot histogram of annual revenue--logistics & comm

ggplot (subset (surveydata, surveydata$ISIC_section=='I')) + 
  geom_histogram (aes (q4_7), 
  binwidth=500000)+scale_x_continuous (limits=c (0,15000000) )+
  labs (x="Takas", title="Annual Revenue 2009- Logistics & Commmunications")  
...
ggplot (aes (x=response, y=q4_7),
  ...
  survey_finc_merged$financesource=='Government bank'))+
  geom_boxplot ()+
...
ggplot (aes (x=response, y=q4_7),
  data=subset (survey_finc_merged,
  financesource=='Microfinance Organization'|!(is.na(response))))+
  geom_boxplot ()+   coord_cartesian (ylim=c (0,17500000)) + 
  scale_x_discrete (labels = function (x) str_wrap (x, width = 15))+
  facet_wrap (~ISIC_sectiondesc)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#opts_chunk$set(fig.width=40, fig.height=40)# bigger plot

survey_transport <- 
  subset (survey_finc_merged,survey_finc_merged$fsabrev=="M" & 
  survey_finc_merged$ISIC_section=='I' & !is.na (response)& !is.na (ISIC4_desc))



survey_transport<-subset (survey_transport, 
  subset= ISIC4_desc %in% c ("Other scheduled passenger land transport",
  "Telecommunications"))


survey_transport$response <- as.factor (survey_transport$response)
#survey_transport$response<-factor(survey_transport$response)

survey_transport$ISIC4_desc<-factor(survey_transport$ISIC4_desc)
#survey_transport$fsabrev<-factor(survey_transport$fsabrev)


#survey_transport<-droplevels((survey_transport)$ISIC4_desc)

#write.csv(survey_transport,"survey_transport.csv")

ggplot (aes (x=response, y=q4_7),
  data=survey_transport)+
  geom_boxplot ()+coord_cartesian (ylim=c (0,20000000)) + 
  scale_x_discrete (labels = function (x) str_wrap (x, width = 15))+
  facet_wrap (~ISIC4_desc,ncol=2)
...
### Plot One
```{r echo=FALSE, options=TRUE, message=FALSE, warning=FALSE, Plot_One}
...
geom_map (data=districtmap, aes (x=long, y=lat,map_id=id), 
 ...
 geom_map (data=aggdata, aes (map_id = dist, fill = q4_7), map = districtmap) + 
  ...
  geom_text (data = MajorCitiesBangladesh, aes (x = Longitude, y = LatitudeName, 
  label = City), size =5, color="red")  +scale_size(range = c(3, 10))
```
...
surveydataselect<-subset (surveydata, 
  subset= ISIC_sectiondesc%in% c ("Manufacturing",
  "Retail & Repair","Hotels and restaurants"))

ggplot (surveydataselect, aes (y=log10 (q4_7), x=q1_12a5a, colour=ISIC_sectiondesc))+
  geom_jitter (alpha=.4) +scale_x_continuous (limits=c (0,20)) +
  geom_smooth (data=subset (surveydataselect,
  surveydataselect$ISIC_sectiondesc=="Manufacturing"),
  aes (y=log10 (q4_7),x=q1_12a5a),method=lm,se=FALSE)+
  scale_fill_discrete (name="ISIC Section")+
  geom_smooth (data=subset (surveydataselect,
  surveydataselect$ISIC_sectiondesc=="Retail & Repair"),
  aes (y=log10 (q4_7),x=q1_12a5a),method=lm,se=FALSE)+
  geom_smooth (data=subset (surveydataselect,
  surveydataselect$ISIC_sectiondesc=="Hotels and restaurants"),
  aes (y=log10 (q4_7),x=q1_12a5a),method=lm,se=FALSE)+
...
First, we observed that annual revenue varied by geography, 
with the 3 districts having the highest average revenue also containing the 3 largest cities in Bangladesh.   This fact supports the hypothesis that 
there is a positive correlation between urbanization and economic activity.  For the purpose of this analysis, the ability to plot this information on a map 
proved to be essential.

We also have a better understanding of how the two measures of business size in this project (annual revenue and number of employees) were positively correlated, 
though not strongly so.  Our analysis of this relationship was enhanced when we broke down the analysis using the ISIC section variable as an indicator
of industry sector, to find that revenue in certain sectors (such as Retail and Repair) had a higher rate of growth as employees increased than in other sectors,
such as Manufacturing.  Box plots were particularly useful for exploring the ISIC section data; further, viewing the business size data on a point plot, colored 
by ISIC section, allowed us to see the differentiated trends of business size across industries.

Additionally, we have a better understanding of financing trends.  Certain sources of finance, such as government banks, tend to lend to firms with higher 
revenue, while others, such as microfinance organisations, tend to focus on firms with low revenue.   These trends in revenue may mean that sources of financing
tend to target certain sectors; government banks lend to a large number of manufacturing firms, as those firms have a high level of revenue.  Boxplots were
used extensively to arrive at these conclusions.
